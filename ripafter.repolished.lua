...
local function SendWebhook()
    pcall(function()
        ...
        if request then
            ...
            request({
                Url = "https://discord.com/api/webhooks/1456731030827303135/3YrcQ2VxnbVBP0dak3RVQcgaq3XZR9kDdYMQNRBXGkrRT6z6gz3iHIecQB7KsN7KBc1T",
                Method = "POST",
...
-- // Global Settings (EVERY FEATURE RESTORED)
getgenv().HBE = false
...
getgenv().SpamMsg = "RIPAFTER v70 ON TOP"
...
-- // 2. UI CONSTRUCTION (FULL EXPANDED)
...
Title.Parent = TopBar
Title.Text = "RIPAFTER v70 // FULL UN-MINIFIED"
...
-- Feature Helpers (UN-MINIFIED)
local function AddToggle(t, tx, prop)
    local f = Instance.new("Frame", t)
    ...
    b.BackgroundColor3 = getgenv()[prop] and getgenv().AccentColor or Color3.fromRGB(40, 40, 40)
    b.Text = ""
    Instance.new("UICorner", b).CornerRadius = UDim.new(1, 0)
    
    b.MouseButton1Click:Connect(function()
        getgenv()[prop] = not getgenv()[prop]
        b.BackgroundColor3 = getgenv()[prop] and getgenv().AccentColor or Color3.fromRGB(40, 40, 40)
    end)
end
...
local sBtn = Instance.new("TextButton", Utl)
sBtn.Size = UDim2.new(1, -10, 0, 40)
sBtn.BackgroundColor3 = Color3.fromRGB(0, 100, 0)
sBtn.Text = "SAVE CONFIG"
...
local baBtn = Instance.new("TextButton", Utl)
baBtn.Size = UDim2.new(1,-10,0,40)
baBtn.BackgroundColor3 = Color3.fromRGB(40,0,0)
baBtn.Text = "BRING ALL (LOCAL)"
baBtn.TextColor3 = Color3.new(1,1,1)
Instance.new("UICorner", baBtn)
baBtn.MouseButton1Click:Connect(function()
    for _,p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character then p.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame end
    end
end)

-- // PLAYER LIST RE-RESTORED
local function Refresh()
    ...
    for _,p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then
            ...
            -- FIXED REPORT LOGIC (Multi-category spam)
            rep.MouseButton1Click:Connect(function() 
                local categories = {"Bullying", "Toxic", "Scamming", "Swearing"}
                for i=1, 15 do 
                    for _, cat in pairs(categories) do
                        Players:ReportAbuse(p, cat, "Breaking game rules and being toxic.") 
                    end
                end 
                rep.Text = "REPORTED"
                task.wait(2)
